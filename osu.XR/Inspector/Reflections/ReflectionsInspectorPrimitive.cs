...
using osu.XR.Inspector.Editors;
using osuTK;
			...
			if ( source.IsValueEditable && source.TargetValue is not Exception ) {
				if ( source.ValueSetter != null && ValueEditor.HasEditorFor( source.TargetType ) ) {
					...
					if ( bindableType is not null && ValueEditor.HasEditorFor( bindableType.GenericTypeArguments[ 0 ] ) ) {
						...
						makeEditable( ValueEditor.GetEditorFor( bindableType.GenericTypeArguments[ 0 ], value ), v => source.TargetValue.SetProperty( "Value", v ) );
