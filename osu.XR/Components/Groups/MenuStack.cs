...
namespace osu.XR.Components.Groups {
	public abstract class MenuStack<T> : CompositeDrawable3D where T : Drawable3D {
		...
		private SettingsSidebar sidebar = new();
		private FlatPanel sidebarPanel = new();

		[Cached]
		OverlayColourProvider colours = new( OverlayColourScheme.Pink );
			...
			Elements.BindCollectionChanged( (_,e) => {
				...
				if ( e.Action == System.Collections.Specialized.NotifyCollectionChangedAction.Add ) {
					foreach ( T i in e.NewItems ) {
						...
						var button = new SidebarIconButton();
						( typeof( SidebarIconButton ).GetField( "iconContainer", BindingFlags.NonPublic | BindingFlags.Instance ).GetValue( button ) as ConstrainedIconContainer ).Icon = (i as IHasIcon)?.CreateIcon() ?? new SpriteIcon { Icon = FontAwesome.Solid.QuestionCircle };
